...
class CategoryService {
  static Future<List<Category>>? getCategories({dynamic filters}) async {
    final responseDocs = await FirestoreService.read(
        'categories', filters != null ? filters : []);
    return (responseDocs as List)
        .map((responseDoc) => Category.fromDocumentSnapshot(responseDoc))
        .toList();
  }
  ...
  static Future<Category?> getCategoryByName(String name) async {
    ...
    final responseDoc =
        await FirestoreService.read('categories', filters, limit: 1);
  ...
  static Future<dynamic>? addCategory(String name) async {
    ...
    if (res != null)
      return ErrorMessage('A category by this name already exists');
  ...
  static Future<dynamic>? deleteCategory(String id) async {
    ...
    if (res.isNotEmpty)
      return ErrorMessage(
          'This category has products! Please delete all products under this category to proceed.');
