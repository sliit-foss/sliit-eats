...
class OrderService {
  static Future<List<Order>>? getOrders({dynamic filters}) async {
    List<dynamic> sorts = [{'name': 'created_at', 'descending': true}];
  ...
  static Future<dynamic>? create(String productId, int quantity) async {
    ...
    return await FirestoreService.write('orders', {'id': UniqueKey().toString(), 'product_id': productId, 'user_id': currentUser!.userId, 'completed': false, 'created_at': Timestamp.now()}, 'Order placed successfully');
  }
}
